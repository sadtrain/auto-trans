...
@Component
public class TBActivityConvertor {

    private static Logger logger = LoggerFactory.getLogger(TBActivityConvertor.class);

    private RestTemplate restTemplate;

    String appSecret = "53aed3457f5fd4f04c72b6378eccb03f";//应用sercret
    String appKey = "623e87e1c0d83"; //应用key
    String host = "https://openapi.dataoke.com/api/tb-service/activity-link?version={version}&appKey={appKey}&pid={pid}&promotionSceneId={promotionSceneId}&sign={sign}";//应用服务接口

    public String convert(String tkl){
        TreeMap<String,String> paraMap = new TreeMap<>();
        paraMap.put("appKey",appKey);
        paraMap.put("promotionSceneId", tkl);
        ...
        ResponseEntity<BaseResponse<DtkActivityLinkResponse>> forEntity = restTemplate.exchange(host, HttpMethod.GET,null, new ParameterizedTypeReference<>() {
        }, paraMap);
//        ResponseEntity<DtkTwdToTwdResponse> forEntity = restTemplate.getForEntity(host, DtkTwdToTwdResponse.class, paraMap);
    ...
    public static void main(String[] args) {
        TBActivityConvertor convertor = new TBActivityConvertor();
        String str = "\n" +
...
//        str = "1￥NrsL263IyXE￥/";
        str = "20150318020008790";


//                        System.out.println(matcher.group(1));
            String myTKL = convertor.convert(str);
            assert myTKL != null;
